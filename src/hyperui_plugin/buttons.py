...
# def simpleAndRevealStaticBorderOnHover(text="Download", href="#", **kwargs):
#     """
#     """
...
def revealHiddenStaticBorderOnHoverBordered(text="Download", href="#", **kwargs):
    with MuCtx:
        with A(classes="group relative inline-block text-sm font-medium", extra_classes = "",
               href=href, **kwargs) as button_box:
            with Span(classes="absolute inset-0 border border-current", extra_classes="btn preset-outlined-surface-500"):
                pass
            with Span(classes="block  px-12 py-3 group-hover:-translate-x-1 group-hover:-translate-y-1",
                      extra_classes="btn preset-outlined-surface-500 transition-transform ",
                      text=text
                      ):
                pass
