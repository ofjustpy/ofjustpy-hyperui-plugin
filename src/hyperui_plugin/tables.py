...
def Simple():
    table = kv.PD.Table(twsty_tags=encode_twstr("min-w-full divide-y-2 divide-gray-200 bg-white text-sm"))
    def add_header():
        arow = kv.PD.Tr(childs=[])
        def add_cell(text, arow = arow):
            cell = kv.PC.Th(twsty_tags=encode_twstr("whitespace-nowrap text-left px-4 py-2 font-medium text-gray-900"), text=text)
            arow.components.append(cell)

        thead = kv.PC.Thead(childs = [arow])
        thead.add_cell = add_cell
        table.components.append(thead)
        return thead
    
    tbody = kv.PD.Tbody(twsty_tags=encode_twstr("divide-y divide-gray-200"))
    ...
    def add_row(**kwargs):
        ...
        def add_cell(text, tr=tr):
            nonlocal font_style
            cell = kv.PC.Td(twsty_tags=encode_twstr(f"whitespace-nowrap px-4 py-2 text-gray-700 {font_style}"), text=text)
            font_style = ""
...
def SimpleWithBorder():
    with MuCtx:
        with Div(classes="overflow-x-auto rounded-lg border border-gray-200") as comp_box:
            with PD.Table(classes="min-w-full divide-y-2 divide-gray-200 bg-white text-sm") as table_box:
                pass
            with PD.Tbody(classes="divide-y divide-gray-200") as tbody_box:
                pass
    def add_header(table_box=table_box):
        with MuCtx:
            with Thead("ltr:text-left rtl:text-right") as header_box:
                with PD.Tr() as row_box:
                    pass
            
        def add_cell(cell_text, row_box=row_box):
            with MuCtx:
                with PD.Th(classes='whitespace-nowrap px-4 py-2 text-left font-medium text-gray-900', text=cell_text) as cell_box:
                    pass
    ...
    def add_row(tbody_box = tbody_box):
        with MuCtx:
            with PD.Tr() as tr_box:
                pass
        def add_cell(text, tr_box=tr_box):
            with MuCtx:
                with PD.Td(classes="whitespace-nowrap px-4 py-2 text-gray-700", text=text) as td_box:
                    pass
