...
def Pills(key):
    with MuCtx:
        with Div() as comp_box:
            with Div(classes='sm:hidden'):
                ...
                with AD.Select(key=key, classes='w-full rounded-md border-gray-200') as select_box:
                    pass

            with Div(classes='hidden sm:block'):
                with PD.Nav(classes='flex gap-6', aria_label='Tabs') as nav_box:
                    pass
    ...
    def add_tab(key, label, selected=False, nav_box=nav_box):
        ...
        select_box.components.append(kv.PD.Option(text=label, selected=selected))
        ...
        with Div() as comp_box:
            ...
            with Div(classes='hidden sm:block'):
                with Div(classes='border-b border-gray-200'):
                    with PD.Nav(classes='-mb-px flex gap-6') as nav_box:
                        pass
...
#         select_comp.components.append(kv.PD.Option(text=text))
        



#     sr_label = kv.PD.Label(#for="Tab",
#         twsty_tags=encode_twstr("sr-only"), text="Tab"
#         )
#     print(encode_twstr("hidden sm:block"))
    
#     root = oj.PD.Div(childs = [kv.PD.Div(childs=[sr_label, select_comp],
#                                          twsty_tags=encode_twstr("sm:hidden")
#                                          ),
#                                kv.PD.Div(childs=[tab_nav_bar],
