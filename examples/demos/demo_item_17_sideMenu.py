...
Nam vel ullamcorper ligula. Ut suscipit bibendum turpis, vitae vestibulum mi sagittis vel. Curabitur a leo vel est ullamcorper luctus. Nunc sit amet fermentum nulla. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Nulla facilisi. Fusce tristique justo et est fermentum, in ullamcorper libero fringilla. Suspendisse potenti. Quisque vel ante nec mi tincidunt eleifend. Vivamus laoreet turpis ac nisi finibus, id efficitur quam egestas. Integer vel tellus urna. Curabitur ut purus vel tellus gravida sodales.
"""
textbox = kv.PD.Halign(kv.PC.Prose(text=main_block_text,
                                   twsty_tags=[prose.lg, twmax/W/"none" ]
                                   ),
                                   twsty_tags=[]
                       )

tlc = kv.HM.Div(childs = [kv.HM.Container(childs=[textbox
                              ],
                      twsty_tags=[mr/x/auto]
                      )
                          ],
                twsty_tags = [flxrsz.one, overflowy.auto]
                )

entire_screen = kv.HM.Div(twsty_tags = [W/full, H/screen, db.f
                                        ],
                          childs = [simple_sideMenu, tlc
                                    

                          ]
                          )
endpoint = kv.create_endpoint("sideMenu",
                              childs = [
                                        entire_screen
                                        ],
                              title="Side Menu",
